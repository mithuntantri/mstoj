    ...
    return {
        ...
        link: function($scope, elem) {
            ...
            $scope.style = {
                ...
                "font-size": $scope.options.style && $scope.options.style.fontSize ? scope.options.style.fontSize : "35px"
            };
            ...
            $scope.setOtpValue = function() {
                ...
                angular.forEach($scope.characters, function(v, k) {
                    ...
                    $scope.options.value = $scope.options.value + v.value.toString();
                });
                if (done) {
                    if (typeof($scope.options.onDone) === "function") {
                        $scope.options.onDone($scope.options.value.toString());
                    }
                }
            };
            ...
            $scope.onKeyUp = function(index, e) {
                ...
                var old = $scope.options.value.toString();
                $scope.setOtpValue();
                if ($scope.characters[index].value.toString().length > 0 && key !== 8 && index != size - 1) {
                    $timeout(function() {
                        elementArr[index + 1][0].focus();
                    });
                }
                if (typeof($scope.options.onChange) === "function" && old !== $scope.options.value.toString()) {
                    $scope.options.onChange($scope.options.value.toString());
                }
            ...
            $scope.onKeyDown = function(index, e) {
                var key = e.keyCode || e.which;
                if (key === 8 && $scope.characters[index].value.toString() === "" && index !== 0) {
                    $timeout(function() {
                        elementArr[index - 1][0].focus();
                    });
                }
            ...
            $timeout(function() {
                for (var i = 0; i < size; i++) {
                    elementArr.push(angular.element(document.querySelector("#otpInput" + i)));
                }
            });
